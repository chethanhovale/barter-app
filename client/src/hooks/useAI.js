...
export function useAI() {
  ...
  // ── Item valuation ────────────────────────────────────────────
  // Accepts: { listing_id } OR { title, description, category, condition, ... }
  // Returns: { estimated_value, confidence, price_range, condition_grade,
  //            depreciation_pct, reasoning }
  const valuateItem = useCallback((params) =>
    call(() => api.post('/ai/trades/valuate', params).then(r => r.data)),
  [call]);

  // ── Hybrid BM25 + vector search ───────────────────────────────
  // Returns: { query, results: [{ id, title, relevance_score, ... }], total }
  const hybridSearch = useCallback((query, options = {}) =>
    call(() => api.post('/ai/search', {
      query,
      top_k:    options.topK    || 8,
      category: options.category || null,
  ...
  return { semanticSearch, enhanceListing, estimateTrade, valuateItem, hybridSearch, loading, error };
}
